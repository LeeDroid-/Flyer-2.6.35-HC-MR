/* linux/arch/arm/mach-msm/board-flyer-mmc.c
 *
...
#define FLYER_SDMC_CD_N_TO_SYS PM8058_GPIO_PM_TO_SYS(FLYER_SDMC_CD_N)
...
static int __init flyer_disablesdcard_setup(char *str)
{
...
__setup("board_flyer.disable_sdcard=", flyer_disablesdcard_setup);
...
static uint32_t flyer_sdslot_switchvdd(struct device *dev, unsigned int vdd)
{
	int i;
...
static unsigned int flyer_sdslot_status(struct device *dev)
{
	unsigned int status;

	status = (unsigned int) gpio_get_value(FLYER_SDMC_CD_N_TO_SYS);
	return (!status);
}

#define FLYER_MMC_VDD		(MMC_VDD_28_29 | MMC_VDD_29_30)

static unsigned int flyer_sdslot_type = MMC_TYPE_SD;
...
static struct mmc_platform_data flyer_sdslot_data = {
	.ocr_mask	= FLYER_MMC_VDD,
	.status_irq	= MSM_GPIO_TO_INT(FLYER_SDMC_CD_N_TO_SYS),
	.status		= flyer_sdslot_status,
	.translate_vdd	= flyer_sdslot_switchvdd,
	.slot_type	= &flyer_sdslot_type,
	.dat0_gpio	= 63,
#ifdef CONFIG_SDIO_CES
	.embedded_sdio		= &flyer_wifi_emb_data,
#endif
};

static unsigned int flyer_emmcslot_type = MMC_TYPE_MMC;
static struct mmc_platform_data flyer_movinand_data = {
	.ocr_mask	= FLYER_MMC_VDD,
	.slot_type	= &flyer_emmcslot_type,
...
static int
flyer_wifi_status_register(void (*callback)(int card_present, void *dev_id),
				void *dev_id)
{
	if (wifi_status_cb)
		return -EAGAIN;
...
static int flyer_wifi_cd;	/* WiFi virtual 'card detect' status */

static unsigned int flyer_wifi_status(struct device *dev)
{
...
static struct mmc_platform_data flyer_wifi_data = {
	...
	.status			= flyer_wifi_status,
	.register_status_notify	= flyer_wifi_status_register,
	.embedded_sdio		= &flyer_wifi_emb_data,
	.dummy52_required = 1,
};

int flyer_wifi_set_carddetect(int val)
{
...
EXPORT_SYMBOL(flyer_wifi_set_carddetect);

int flyer_wifi_power(int on)
{
	printk(KERN_INFO "%s: %d\n", __func__, on);
...
EXPORT_SYMBOL(flyer_wifi_power);

int flyer_wifi_reset(int on)
{
	printk(KERN_INFO "%s: do nothing\n", __func__);
	return 0;
}

int __init flyer_init_mmc(unsigned int sys_rev)
{
	uint32_t id;
	wifi_status_cb = NULL;
	sdslot_vreg_enabled = 0;
	...
	msm_add_sdcc(3, &flyer_wifi_data, 0, 0);
	...
	set_irq_wake(MSM_GPIO_TO_INT(FLYER_SDMC_CD_N_TO_SYS), 1);
