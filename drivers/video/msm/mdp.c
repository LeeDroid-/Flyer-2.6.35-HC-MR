...
static int locked_enable_mdp_irq(struct mdp_info *mdp, uint32_t mask)
{
	...
	if (!mdp_irq_mask) {
		...
		if (mdp->state & MDP_STATE_STANDBY) {
			...
			if (hdmi_common_state->hpd_state)
				clk_set_rate(mdp->ebi1_clk, UINT_MAX);
			else
	     ...
	     struct fb_cmap *cmap)
{
	struct mdp_info *mdp = container_of(mdp_dev, struct mdp_info, mdp_dev);
		...
		addr = 0x94800 + cmap->start * 4 + i * 4;
		val = ((*g & 0xff) |((*b & 0xff) << 8) | ((*r & 0xff) << 16));
		mdp_writel(mdp, val, addr);
        }

	/* lcdc case */
	val = mdp_readl(mdp, 0x90070);
	val |= 0x7;
        mdp_writel(mdp, val, 0x90070);
...
static void mdp_lut_enable(struct mdp_info *mdp)/* non lcdc case */
{
        if (mdp_lut_push) {
                mutex_lock(&mdp_lut_push_sem);
                mdp_lut_push = 0;
...
void mdp_pipe_kickoff(struct mdp_info *mdp, uint32_t term)/* non lcdc case */
{
...
static int mdp_do_histogram(struct mdp_device *mdp_dev, struct mdp_histogram *hist, struct mdp_histogram *out)
{
	...
	wait_for_completion_killable_timeout(&mdp_hist_comp, msecs_to_jiffies(300));
